...
2. Установите зависимости (обязательно с модулем job-queue для таймеров):
   ```bash
...
7. Добавьте вашего бота в чаты, которые хотите отслеживать, и **обязательно выдайте ему права администратора на "Удаление сообщений"**.

8. **Активация в чате:** Чтобы бот начал удалять сообщения, администратор (указанный в `admins.json` или `config.json`) должен написать команду `/addchat` прямо в целевой группе. Без этого бот будет игнорировать группу.

## Как отлаживать ошибки
Если бот не удаляет сообщения, посмотрите логи службы:
```bash
sudo journalctl -u remove-join-messages-bot.service -f
```
Там будут отображены ошибки, если боту не хватает прав администратора.
